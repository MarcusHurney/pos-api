...
module.exports = app => {
  ...
  app.get('/fetchAllEmployees/:userId', function(req, res, next) {
    Employee.find({ administrator: req.params.userId }).exec(function(
      err,
      employees
    ) {
      //check for error
  ...
  app.post('/addNewEmployee', function(req, res, next) {
    ...
    User.findById(req.body.adminId).then(
      user => {
  ...
  app.get('/employeeTodayRevenue/:employeeId', function(req, res, next) {
    ...
    Sale.find({
      soldByEmployee: req.params.employeeId,
  ...
  app.get('/fetchSingleEmployee/:employeeId', function(req, res, next) {
    Employee.findById(req.params.employeeId).then(
      employee => {
      ...
      [
        ...
        function(employee, done) {
          var smtpTransport = nodemailer.createTransport('SMTP', {
            service: 'SendGrid',
            auth: {
              user: procces.env.SEND_USER,
              pass: procces.env.SEND_PASS
            }
          });
          var mailOptions = {
            to: employee.email,
