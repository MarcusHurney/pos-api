...
module.exports = app => {
  app.get('/loadTodaysTopItems/:userId', function(req, res, next) {
    ...
    Sale.find({
      ...
      var todaysTopItemsBySaleFrequency = Object.keys(occurrences)
        .map(itemId => ({
  ...
  app.get('/loadWeeksTopItems/:userId', function(req, res, next) {
    ...
    Sale.find({
      owner: req.params.userId,
      created_at: { $gte: sevenDaysAgo, $lt: endOfToday }
      ...
      var weeksTopItemsBySaleFrequency = Object.keys(occurrences)
        .map(itemId => ({
          item: occurrences[itemId].item,
          occurrences: occurrences[itemId].occurrences
        }))
  ...
  app.get('/loadMonthsTopItems/:userId', function(req, res, next) {
    ...
    Sale.find({
      ...
      var monthsTopItemsBySaleFrequency = Object.keys(occurrences)
        .map(itemId => ({
          item: occurrences[itemId].item,
          occurrences: occurrences[itemId].occurrences
        }))
  ...
  app.get('/loadTodaysRevenue/:userId', function(req, res, next) {
    ...
    Sale.find({ owner: req.params.userId }).exec(function(err, allSales) {
      if (err) {
        return next(err);
      } //check for error
      ...
      allSales.map(sale => {
        ...
        if (
          sale.date.day === currentDay &&
          sale.date.month === currentMonth &&
          sale.date.year === currentYear
        ) {
    ...
    Sale.find({ owner: req.params.userId }).exec(function(err, allSales) {
      ...
      allSales.map(sale => {
        if (sale.date.year === currentYear) {
          ...
          if (sale.date.month === 0) {
            janRevenue += sale.total;
          } else if (sale.date.month === 1) {
            febRevenue += sale.total;
          } else if (sale.date.month === 2) {
            marchRevenue += sale.total;
          } else if (sale.date.month === 3) {
            aprilRevenue += sale.total;
          } else if (sale.date.month === 4) {
            mayRevenue += sale.total;
          } else if (sale.date.month === 5) {
            juneRevenue += sale.total;
          } else if (sale.date.month === 6) {
            julyRevenue += sale.total;
          } else if (sale.date.month === 7) {
            augRevenue += sale.total;
          } else if (sale.date.month === 8) {
            septRevenue += sale.total;
          } else if (sale.date.month === 9) {
            octRevenue += sale.total;
          } else if (sale.date.month === 10) {
            novRevenue += sale.total;
          } else if (sale.date.month === 11) {
            decRevenue += sale.total;
          }
        } else {
          return;
        }
    ...
    parallel(
      [
        function(callback) {
          ...
          Sale.find({
            owner: req.params.userId,
            created_at: { $gte: sevenDaysAgo, $lt: sixDaysAgo }
        ...
        function(callback) {
          ...
          Sale.find({
            owner: req.params.userId,
            created_at: { $gte: sixDaysAgo, $lt: fiveDaysAgo }
          }).exec(function(err, sales) {
            if (err) {
              return callback(err);
            } //check for error
        ...
        function(callback) {
          ...
          Sale.find({
            owner: req.params.userId,
            created_at: { $gte: fiveDaysAgo, $lt: fourDaysAgo }
          }).exec(function(err, sales) {
            if (err) {
              return callback(err);
            } //check for error
        ...
        function(callback) {
          ...
          Sale.find({
            owner: req.params.userId,
            created_at: { $gte: threeDaysAgo, $lt: twoDaysAgo }
          }).exec(function(err, sales) {
            if (err) {
              return callback(err);
            } //check for error
        ...
        function(callback) {
          ...
          Sale.find({
            owner: req.params.userId,
            created_at: { $gte: oneDayAgo, $lt: startOfToday }
          }).exec(function(err, sales) {
            if (err) {
              return callback(err);
            } //check for error
