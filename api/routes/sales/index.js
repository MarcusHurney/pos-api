...
module.exports = app => {
  ...
  app.get('/fetchSales/:userId', function(req, res, next) {
    Sale.find({ owner: req.params.userId }).exec(function(err, sales) {
      //check for error
      if (err) {
        return next(err);
      }
  ...
  app.get('/fetchSalesByDate/:userId/:startDate/:endDate', function(
    req,
    res,
    next
  ) {
  ...
  app.get('/fetchSale/:saleId', function(req, res, next) {
    Sale.findById(req.params.saleId).exec(function(err, sale) {
      // check for error
      if (err) {
        return next(err);
      }
  ...
  app.delete(`/undoSale/:saleId`, function(req, res, next) {
    Sale.findByIdAndRemove(req.params.saleId, (err, deletedSale) => {
      if (err) {
        return next(err);
      }
